...
function show(data)
{
  ...
  while (i < data.length)
  {
    ...
    if(data[i].dead == 1)
      char = '<img src=\'images/map/dead.gif\' style=\'float:center\' border=0 width=18 height=18\>';
    else
      char = '<img src=\'images/races/'+data[i].race+'-'+data[i].gender+'.gif\' style=\'float:center\' border=0 width=18 height=18\>';
    n=0;
    ...
    if(n == point_count)
    {
      ...
      if(in_array(data[i].map, maps_array))
      {
        ...
        mpoints[point_count].single_text = data[i].zone+'<br\>'+data[i].level+' lvl<br\>'+char+'&nbsp;<img src=\'images/classes/'+data[i].cl+'.gif\' style=\'float:center\' border=0 width=18 height=18\><br\>'+race_name[data[i].race]+'<br/>'+class_name[data[i].cl]+'<br/>';
  ...
  while(n!=point_count)
  {
    if(!in_array(mpoints[n].map_id, maps_array))
      instances[mpoints[n].Extention] += '<img src="images/map/inst-icon.gif" style="position: absolute; border: 0px; left: '+instances_x[mpoints[n].Extention][mpoints[n].map_id]+'px; top: '+instances_y[mpoints[n].Extention][mpoints[n].map_id]+'px;" onMouseMove="tip(mpoints['+n+'],1,false);" onMouseDown="tip(mpoints['+n+'],1,true);" onMouseOut="h_tip();mpoints['+n+'].multi_text.current=0;"\>';
    else if(mpoints[n].player > 1)
      groups[mpoints[n].Extention] += '<img src="images/map/group-icon.gif" style="position: absolute; border: 0px; left: '+mpoints[n].x+'px; top: '+mpoints[n].y+'px;" onMouseMove="tip(mpoints['+n+'],1,false);" onMouseDown="tip(mpoints['+n+'],1,true);" onMouseOut="h_tip();mpoints['+n+'].multi_text.current=0;"\>';
    else
    {
      if(mpoints[n].faction)
        point = "images/map/horde.gif";
      else
        point = "images/map/allia.gif";
  ...
  document.getElementById("server_info").innerHTML='\'<?php echo $realm_name ?>\' online: <b style="color: rgb(100,100,100);" onMouseMove="tip(\'<tr\><td\><img src=\\\'images/map/hordeicon.gif\\\'\></td\><td\><b style=\\\'color: rgb(210,50,30);\\\'\><?php echo $lang_defs['faction'][1]; ?>:</b\> <b\>'+total_players_count[1]+'</b\></td\></tr\><tr\><td\><img src=\\\'images/map/allianceicon.gif\\\'\></td\><td\><b style=\\\'color: rgb(0,150,190);\\\'\><?php echo $lang_defs['faction'][0]; ?>:</b\> <b\>'+total_players_count[0]+'</b\></td\></tr\>\',2,false);" onMouseOut="h_tip();"><?php echo $lang_defs['total']; ?></b> '+eval(total_players_count[0]+total_players_count[1])+'';
  for(i = 0; i < maps_count; i++)
  {
    document.getElementById("server_info").innerHTML += '&nbsp;<b style="color: rgb(160,160,20); cursor:pointer;" onClick="switchworld('+i+');" onMouseMove="tip(\'<tr\><td\><img src=\\\'images/map/hordeicon.gif\\\'\></td\><td\><b style=\\\'color: rgb(210,50,30);\\\'\><?php echo $lang_defs['faction'][1]; ?>:</b\> <b\>'+horde_count[i]+'</b\></td\></tr\><tr\><td\><img src=\\\'images/map/allianceicon.gif\\\'\></td\><td\><b style=\\\'color: rgb(0,150,190);\\\'\><?php echo $lang_defs['faction'][0]; ?>:</b\> <b\>'+alliance_count[i]+'</b\></td\></tr\>\',2,false);" onMouseOut="h_tip();">'+maps_name_array[i]+'</b\> '+players_count[i]+'';
  }
}
...
function load_data()
{
      ...
      if(show_status)
      {
        if(req.responseJS.status)
        {
          ...
          if(status_data[2] != req.responseJS.status.online)
          {
            ...
            if(status_data[2] == 0)
            {
              obj.src = "images/map/realm_off.gif"
            }
            else
              obj.src = "images/map/realm_on.gif"
          }
  ...
  req.open('GET', 'kernel/map_core.php', true);
  req.send({ });
}
	...
	<div ID="info">
		<center>
			<table border="0" cellspacing="0" cellpadding="0" height="20">
				<tr>
					<td valign="top" id="timer"></td>
				</tr>
			</table>
		</center>
	</div>
	<div id="must_login">
		<img src="images/map/mustlogin.gif">
	</div>
	<div ID="info_bottom">
		<center>
