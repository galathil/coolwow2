...
require_once("map_fonc.php");
require_once("../lang/french.php");
include_once("../lang/map_zone_name_fr.php");
$royaume = $_SESSION['royaume'];

$maps_count = count($lang_defs['maps_names']);
$mangos_db = new DBLayer($characters[$royaume]['host'], $characters[$royaume]['user'], $characters[$royaume]['password'], $characters[$royaume]['db']);
$mangos_db->query("SET NAMES ".$characters[$royaume]['encoding']."");
...
require_once "../scripts/JsHttpRequest/Php.php";
...
if(test_realm()) {
	...
	while($result = $mangos_db->fetch_assoc($query)) {
		...
		$arr[$i]['zone']=get_zone_name2($result['zone']);
